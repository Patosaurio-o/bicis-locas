function validateForm(){	
	...
	if(nombre == ""){
		$('#name').parent().append('<span class="error">deve llenar campo obligatorio Ej: "Rosamel"</span>');
	} 
		...
		$('#name').parent().append('<span class="error">su nombre deve iniciar con mayusculas Ej: "Rosamel"</span>');
	}
	if(apellido == ""){
		$('#lastname').parent().append('<span class="error">deve llenar campo obligatorio Ej: "Fierro"</span>');
	}
	else if(apellido[0].toUpperCase() != apellido[0]){
		$('#lastname').parent().append('<span class="error">su apellido deve iniciar con mayusculas Ej: "Fierro"</span>');
	}
	else if(/^[A-Za-z]+$/.test(apellido) == false){
		$('#lastname').parent().append('<span class="error">ingresar solo letras</span>');
	} 
	if(email == ""){
		$('#input-email').parent().append('<span class="error">deve llenar campo obligatorio Ej: "rosa.fier.del@algo.com"</span>');
	}
	else if(validarEmail(email) == false){
		$('#input-email').parent().append('<span class="error">el email ingresado es invalido</span>');
	}
	if(passwd == ""){
		$('#input-password').parent().append('<span class="error">deve llenar campo obligatorio</span>');
	}
